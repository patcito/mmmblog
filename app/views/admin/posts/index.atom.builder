atom_feed(
  ...
  feed.generator "Enki", "uri" => AppConfig.site
  ...
  @posts.each do |post|
   feed.entry(post, :url => custom_posts(:author=> @author, :tag => @tag, :only_path => false), :published => post.published_at, :updated => post.edited_at) do |entry|
